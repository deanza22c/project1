...
****(needsQueue to be present)void breadth_firstTraversal()								- print breadth first traversal to screen
...
protected:
	...
	DualLinkDataNode<T> *addInOrder(DualLinkDataNode<T> *subTreePtr, DualLinkDataNode<T> *newNode)
	{
		...
		else
		{
			...
			cout << subTreePtr->data << endl;
			cout << (newNode->data <= subTreePtr->data) << endl << endl;
	...
	//  Once both paths are checked, print out the data in the node at the end of the branch
	//  Also this function is protected from access outside of the class
	void cout_post_orderTraversal(DualLinkDataNode<T> *currentRoot)
	{
		// if the currentRoot is null, that means this branch's patch is at the end
		if (currentRoot == nullptr)
		{
		...
		{
			cout_post_orderTraversal(currentRoot->leftBranch); // check the left branch for a valid path
			cout_post_orderTraversal(currentRoot->rightBranch); // check the right branch for a valid path
			cout << currentRoot->data << std::endl; // when both branches are nullptr's, print out the current node's data
		}
	}


	void cout_pre_orderTraversal(DualLinkDataNode<T> *currentRoot)
	{
		// if the currentRoot is null, that means this branch's patch is at the end
		if (currentRoot == nullptr) return;
		cout << currentRoot->data << std::endl; // when both branches are nullptr's, print out the current node's data
		cout_pre_orderTraversal(currentRoot->leftBranch); // check the left branch for a valid path
		cout_pre_orderTraversal(currentRoot->rightBranch); // check the right branch for a valid path
	}

	void cout_In_orderTraversal(DualLinkDataNode<T> *currentRoot)
	{
		// if the currentRoot is null, that means this branch's patch is at the end
		if (currentRoot == nullptr) return;
		cout_In_orderTraversal(currentRoot->leftBranch); // check the left branch for a valid path
		cout << currentRoot->data << std::endl; // when both branches are nullptr's, print out the current node's data
...
public:
	...
	*/
	...
	//  uses post order traveresal to process the items in the tree, then print the data to the &writeFile
	void coutPostOrderTraverse()
	{
		if (!rootNode)
		{
			std::cout << "error: tree is empty";
		...
		{
			cout_post_orderTraversal(rootNode);
		}
	}

	//  uses post order traveresal to process the items in the tree, then print the data to the &writeFile
	void coutPreOrderTraverse()
	{
		if (!rootNode)
		{
			std::cout << "error: tree is empty";
		...
		{
			cout_pre_orderTraversal(rootNode);
		}
	}
	//  uses post order traveresal to process the items in the tree, then print the data to the &writeFile
	void coutInOrderTraverse()
	{
		if (!rootNode)
		{
			std::cout << "error: tree is empty";
